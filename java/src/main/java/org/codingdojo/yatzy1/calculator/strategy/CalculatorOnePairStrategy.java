...
public class CalculatorOnePairStrategy implements CalculatorPairStrategy {
    @Override
    public int calculateScoreForPairOfDice(int d1, int d2, int d3, int d4, int d5) {
        ...
        return pairs.entrySet().stream().filter(a -> a.getValue() == 2).mapToInt(d -> (int) (d.getKey() * d.getValue())).max().orElse(0);
    }
}
